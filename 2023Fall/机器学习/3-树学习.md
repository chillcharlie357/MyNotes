...
从一个假设空间中搜索一个正确拟合训练样例的假设。  搜索的假设空间就是可能的决策树的集合。  从简单到复杂的爬山算法遍历假设空间。从空的树开始，然后逐步 考虑更加复杂的假设。引导爬山搜索的评估函数是信息增益度量
...
4. 如果**Attributes为“空”**，那么返回单结点树Root，label设置 为Examples中最普遍的目标属性值
5. **否则**
